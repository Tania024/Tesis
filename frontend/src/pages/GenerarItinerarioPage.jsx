...
const GenerarItinerarioPage = () => {
  ...
  // ✅ NUEVOS ESTADOS: Tipo de entrada y acompañantes
  const [tipoEntrada, setTipoEntrada] = useState('individual');
  const [acompanantes, setAcompanantes] = useState(0);
  
  // ✅ NUEVOS ESTADOS: Selección manual de áreas
  ...
  const cargarPerfil = async () => {
    try {
      ...
      if (data) {
        ...
        // ✅ INICIALIZAR ÁREAS SELECCIONADAS CON INTERESES DETECTADOS
        ...
        setAreasSeleccionadas(['ARQ-01', 'ETN-01', 'ART-01']); // Por defecto
      }
    } catch (err) {
  ...
  // ✅ MAPEO DE ÁREAS A INTERESES PARA EL BACKEND
  ...
  // ✅ TOGGLE DE SELECCIÓN MANUAL
  const toggleSeleccionManual = () => {
    setSeleccionManual(!seleccionManual);
    
    if (!seleccionManual) {
      // Al activar selección manual, copiar intereses detectados
  ...
  const AREAS_DISPONIBLES = [
    { 
      code: 'ARQ-01', 
      nombre: 'Sala Arqueológica Cañari', 
      icon: '🏺', 
      color: 'bg-orange-100 text-orange-700',
      desc: 'Descubre la historia prehispánica del pueblo Cañari'
    },
    { 
      code: 'ETN-01', 
      nombre: 'Sala Etnográfica', 
      icon: '🎭', 
      color: 'bg-purple-100 text-purple-700',
      desc: 'Conoce las tradiciones y cultura de los pueblos andinos'
    },
    { 
      code: 'AVE-01', 
      nombre: 'Aviario de Aves Andinas', 
      icon: '🦅', 
      color: 'bg-blue-100 text-blue-700',
      desc: 'Observa la diversidad de aves nativas de los Andes'
    },
    { 
      code: 'BOT-01', 
      ...
      desc: 'Explora la flora nativa del Ecuador'
    },
    { 
      code: 'ART-01', 
      ...
      desc: 'Admira obras de arte del período colonial'
    },
    { 
      code: 'RUIN-01', 
      nombre: 'Parque Arqueológico Pumapungo', 
      icon: '🏛️', 
      color: 'bg-yellow-100 text-yellow-700',
      desc: 'Recorre el parque para observar el antiguo complejo inca'
    },
    { 
      code: 'TEMP-01', 
      nombre: 'Exhibición Temporal', 
      icon: '🎪', 
      color: 'bg-pink-100 text-pink-700',
      desc: 'Descubre exposiciones especiales y rotativas'
    }
  ];
  ...
  const handleGenerarItinerario = async () => {
    try {
      ...
      // 🔥 VALIDACIÓN 1: Al menos una área seleccionada
      if (areasSeleccionadas.length === 0) {
        setError('Por favor, selecciona al menos una área para visitar');
        setLoading(false);
        return;
      }

      // 🔥 VALIDACIÓN 2: Tipo de entrada vs acompañantes
      if (tipoEntrada === 'grupo' && acompanantes < 1) {
        setError('⚠️ Los grupos deben tener al menos 1 acompañante');
        setLoading(false);
        return;
      }

      if (tipoEntrada === 'individual' && acompanantes > 0) {
        setError('⚠️ La entrada individual no puede tener acompañantes');
        setLoading(false);
        return;
      }
      ...
      // ✅ USAR ÁREAS SELECCIONADAS MANUALMENTE
      ...
      const payload = {
        ...
        tipo_entrada: tipoEntrada,           // 🔥 CAMPO NUEVO
        acompanantes: acompanantes,          // 🔥 CAMPO NUEVO
        ...
        <div className="card">
          ...
          <div className="mb-8">
            <h3 className="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2">
              <span>🏛️</span>
              <span>Selecciona las áreas que quieres visitar</span>
            </h3>
            
            {/* Toggle para selección manual */}
            <div className="mb-4 p-4 bg-purple-50 border-2 border-purple-200 rounded-lg">
              <label className="flex items-start gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  checked={seleccionManual}
                  onChange={toggleSeleccionManual}
                  className="mt-1 w-5 h-5 text-purple-600 rounded focus:ring-purple-500"
                />
                <div>
                  <p className="font-semibold text-purple-900">
                    ✨ Quiero seleccionar las áreas manualmente
                  </p>
                  <p className="text-sm text-purple-700 mt-1">
                    {seleccionManual 
                      ? 'Estás seleccionando áreas manualmente' 
                      : 'Usaré tus intereses detectados para recomendarte áreas'}
                  </p>
                </div>
              </label>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`p-4 rounded-lg border-2 transition-all text-left ${
                      isSelected
                        ? `${area.color} border-purple-500 ring-2 ring-purple-500`
                        : 'bg-white border-gray-200 hover:border-gray-300'
                    } ${!seleccionManual ? 'opacity-50 cursor-not-allowed' : ''}`}
                  >
            ...
            {seleccionManual && areasSeleccionadas.length === 0 && (
              <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 text-sm">
                <div className="flex items-start gap-2">
                  ...
                  <span>Debes seleccionar al menos una área para continuar</span>
                </div>
              </div>
            )}

            {/* Resumen de áreas seleccionadas */}
            {areasSeleccionadas.length > 0 && (
              <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
                <p className="text-sm font-medium text-green-800">
                  📌 Áreas seleccionadas ({areasSeleccionadas.length}):
                </p>
          ...
          <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg mb-6">
            ...
            <ul className="space-y-1 text-sm text-gray-700">
              <li>
                <span className="font-medium">Áreas:</span> {areasSeleccionadas.length} seleccionadas
              </li>
              <li>
                <span className="font-medium">Entrada:</span> {tipoEntrada.charAt(0).toUpperCase() + tipoEntrada.slice(1)}
              </li>
              <li>
                <span className="font-medium">Acompañantes:</span> {acompanantes} {acompanantes === 1 ? 'persona' : 'personas'}
              </li>
              <li>
          ...
          <button
            ...
            disabled={loading || (seleccionManual && areasSeleccionadas.length === 0)}
          ...
          {seleccionManual && areasSeleccionadas.length === 0 && (
            <p className="text-center text-sm text-red-500 mt-3">
              Por favor, selecciona al menos una área para continuar
            </p>
          )}
        </div>
      </div>
